...
"""yelp"""
# 读取CSV文件
csv_file_path = 'dataraw/yelp_train.csv'
train_df = pd.read_csv(csv_file_path)
train_df = shuffle(train_df)

# 获取train中唯一的项目和类别
unique_projects_train = train_df['business_id2'].unique()
unique_categories_train = train_df['classes'].unique()
unique_users_train = train_df['user_id2'].unique()

# 总文件
csv_file_path = 'dataraw/yelp.csv'
all_df = pd.read_csv(csv_file_path)
# 获取总项目和类别的数量
num_projects = len(all_df['business_id2'].unique())
num_categories = len(all_df['classes'].unique())
num_users = len(all_df['user_id2'].unique())
...
# 填充关系矩阵
for _, row in train_df.iterrows():
    project_index = row['business_id2']
    category_index = row['classes']
...
for _, row in train_df.iterrows():
    project_user_dict[row['business_id2']].append(row['user_id2'])
# 创建一个列表，用于存储用户组合
user_combinations = []
# 遍历字典，获取用户组合
...
test_data = []
# 读取test.csv文件
test_file_path = 'dataraw/yelp_test.csv'
test_df = pd.read_csv(test_file_path)
all_projects = [i for i in range(0,num_projects)]
# 提取用户、项目和类别，形成列表
test_Data = test_df[['user_id2', 'business_id2']].values.tolist()
n = len(test_Data)
...
for i in range(n):
    ...
    # Negative Sample
    user_interacted_projects = train_df[train_df['user_id2'] == u]['business_id2'].unique()
    user_interacted_projects_test = test_df[test_df['user_id2'] == u]['business_id2'].unique()
