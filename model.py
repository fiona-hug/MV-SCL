...
class MODEL(nn.Module):
    ...
    def forward(self, iftraining, uid, iid, norm = 1):
        ...
        for i in range(len(self.encoder)):
            ...
            userEmbeddings = self.ui_userEmbedding0
            itemEmbeddings = self.ui_itemEmbedding0
            uiEmbeddings=torch.cat([userEmbeddings0,itemEmbeddings0],0)
            if norm == 1:
                norm_embeddings = F.normalize(userEmbeddings0, p=2, dim=1)###L2范数标准化
...
class GCN_layer(nn.Module):
    ...
    def sparse_mx_to_torch_sparse_tensor(self, sparse_mx):
        """Convert a scipy sparse matrix to a torch sparse tensor将scipy稀疏矩阵转换为torch稀疏张量."""
        if type(sparse_mx) != sp.coo_matrix:
            sparse_mx = sparse_mx.tocoo().astype(np.float32)
        indices = torch.from_numpy(
            np.vstack((sparse_mx.row, sparse_mx.col)).astype(np.int64))
    ...
    def forward(self, features, Mat, index):
        ...
        """自注意力网络GPU"""
        W_query = torch.nn.Linear(32, 32).cuda()
        W_key = torch.nn.Linear(32, 32).cuda()
        # query = W_query(subset_features)
        # key = W_key(subset_features)
        #
        # attention_scores = torch.matmul(query, key.transpose(-2, -1))
        # attention_scores = attention_scores / torch.sqrt(torch.tensor(32, dtype=torch.float32))
        # attention_weights = F.softmax(attention_scores, dim=-1)
        # attention_output = torch.matmul(attention_weights, subset_features)
        # out_features = torch.spmm(subset_sparse_tensor, attention_output )
        ...
        batch_size = 1000  # 举例，根据实际情况调整

        new_features = torch.empty(features.shape).cuda()
        for batch_start in range(0, features.shape[0], batch_size):
            batch_end = min(batch_start + batch_size, features.shape[0])
            batch_features = features[batch_start:batch_end].cuda()

            # 以下是处理批次的代码，根据需要调整
            query = W_query(batch_features)
            key = W_key(batch_features)
            attention_scores = torch.matmul(query, key.transpose(-2, -1))
            attention_scores /= torch.sqrt(torch.tensor(32, dtype=torch.float32).cuda())
            attention_weights = F.softmax(attention_scores, dim=-1)
            attention_output = torch.matmul(attention_weights, batch_features)

            # 使用稀疏矩阵乘法时，需要注意batch的索引
            # 此处省略了sparse_tensor与attention_output的乘法过程，因为需要根据具体情况进行调整

            new_features[batch_start:batch_end] = attention_output
            # 注意保持不需要更新的部分
            if batch_start == 0:
                dif_index = np.setdiff1d(torch.arange(features.shape[0]), index)
                new_features[dif_index] = features[dif_index].cuda()
        ...
        # batch_size,output_dim, input_dim = features.size()
        # linear_layer = nn.Linear(input_dim, output_dim).cuda()
        # node_weights = linear_layer(features).squeeze(0)
        #
        # attention_output = torch.matmul(node_weights, subset_features)
        # out_features = torch.spmm(subset_sparse_tensor, attention_output)
        #
        # new_features = torch.empty(subset_features.shape).cuda()
        # new_features[index] = out_features
        # dif_index = np.setdiff1d(torch.arange(subset_features.shape[0]), index)
        # new_features[dif_index] = subset_features[dif_index]
        return new_features.cuda()
